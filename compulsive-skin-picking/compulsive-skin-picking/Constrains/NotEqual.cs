	...
	namespace Constrains {
		...
		public class NotEqual: AbstractConstrain {
				...
				foreach (var trigger in triggers) {
					if (trigger.type == PropagationTrigger.Type.Assign) {
						if (trigger.variable == a) {
							if (assignment[b].Ground && assignment[b].Value == trigger.value) {
						...
						if (trigger.variable == b) {
							if (assignment[a].Ground && assignment[a].Value == trigger.value) {
			...
			public override string ToString() {
				return string.Format("[{0} != {1}]", a.Identifier, b.Identifier);
			}
